...
**GOOG 3m OHLCV stocks historical data** — ultra high-quality 3m OHLCV for **Alphabet**. Clean `time, open, high, low, close, volume` CSV for backtesting, algorithmic trading and quantitative research.
...
df = pd.read_csv('GOOG_3m.csv', parse_dates=['time'])
